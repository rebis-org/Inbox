...
function useEmailMutation<TVars extends { mailboxId: string }, TData = unknown>(
...
export function useSendEmail() {
  return useEmailMutation(({ mailboxId, email }: { mailboxId: string, email: unknown }) => api.sendEmail(mailboxId, email));
}
...
export function useMarkThreadRead() {
  return useEmailMutation(({ mailboxId, threadId }: { mailboxId: string, threadId: string }) => api.markThreadRead(mailboxId, threadId));
}

export function useDeleteEmail() {
  return useEmailMutation(({ mailboxId, id }: { mailboxId: string, id: string }) => api.deleteEmail(mailboxId, id));
}
...
// Single residence for "invalidate what a folder listing shows": routes and
// hooks call this instead of touching the query client themselves.
export function useRefreshEmails(mailboxId: string): () => void {
  const queryClient = useQueryClient();
  return useCallback(() => {
    queryClient.invalidateQueries({ queryKey: ['emails', mailboxId] });
