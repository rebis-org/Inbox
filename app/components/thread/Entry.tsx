...
export default function Entry({
  ...
  const senderLabel = isDraft ? m.viewDraftReply() : (isSelf ? m.viewYou() : email.sender);
          ...
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium text-foreground truncate">{senderLabel}</span>
                {isDraft && <Badge variant="outline">{m.viewDraft()}</Badge>}
              </div>
              <div className="text-xs text-muted-foreground">{m.viewToPrefix()}{email.recipient}</div>
            </div>
          </div>
