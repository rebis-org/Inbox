...
function HomeRoute() {
  ...
  const domainItems = [
    { label: m.homeSelectDomain(), value: null },
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      toast.add({ title: m.homeCreated() });
      setIsCreateOpen(false);
      setNewPrefix('');
      setNewName('');
    } catch (err: unknown) {
      const message = (err instanceof Error ? err.message : null) || m.homeCreateFailed();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.add({ title: m.homeDeleted() });
      setIsDeleteOpen(false);
      setMailboxToDelete(null);
    } catch {
      toast.add({ title: m.homeDeleteFailed(), type: 'error' });
    } finally {
      setIsDeleting(false);
    }
  };
    ...
    <div className="min-h-dvh bg-muted">
      <div className="mx-auto max-w-2xl px-4 py-8 md:px-6 md:py-16">
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-foreground">{m.homeTitle()}</h1>
            {!isConfigured && (
          ...
          : (accounts.length > 0
                  ...
                  >
                    ...
                    {!isConfigured && (
                      <Button
                        variant="ghost"
                        size="icon-sm"
                        aria-label={m.homeDeleteAria({ email: account.email })}
                        onClick={(e) => {
                          e.preventDefault();
                          e.stopPropagation();
                          setMailboxToDelete({
            ...
            : (
              <State
                ...
                title={m.homeEmptyTitle()}
                desc={isConfigured ? m.homeEmptyConfigured() : m.homeEmptyUnconfigured()}
                action={!isConfigured && (
                  <Button variant="default" onClick={() => setIsCreateOpen(true)}>
                    <PlusIcon data-icon="inline-start" />
                    {m.homeCreateCta()}
      ...
      <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
        <DialogContent className="p-6 sm:max-w-xs">
          <DialogTitle className="text-base font-semibold mb-5">{m.homeCreateTitle()}</DialogTitle>
          <form onSubmit={handleCreate} className="flex flex-col gap-4">
            {createError && <p className="text-sm text-destructive">{createError}</p>}
            <Field>
              <FieldLabel htmlFor="address-prefix">{m.homeEmailAddress()}</FieldLabel>
                ...
                {domains.length > 1
                  ...
                  : (
                    <span className="text-sm text-muted-foreground">
                      {selectedDomain || m.homeNoDomain()}
                    </span>
                  )}
              </div>
            </Field>
            <Field>
              <FieldLabel htmlFor="display-name">{m.homeDisplayName()}</FieldLabel>
              <Input
                id="display-name"
                placeholder="Info"
                value={newName}
      ...
      >
        <DialogContent className="p-6 sm:max-w-xs">
          <DialogTitle className="text-base font-semibold mb-2">{m.homeDeleteTitle()}</DialogTitle>
          <DialogDescription className="text-muted-foreground text-sm mb-5">
            {mailboxToDelete && m.homeDeleteDesc({ email: mailboxToDelete.email })}
          </DialogDescription>
