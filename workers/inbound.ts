...
const AUTH_PASS_REGEX = /(?:^|[;\s])dmarc=pass\b/i;
const AUTH_FAIL_REGEX = /(?:^|[;\s])(?:dmarc|spf)=fail\b/i;
...
function senderAuthStatus(headers: Array<{ key: string, value: string }>): string {
  const authResults = headers.find((header) => header.key === 'authentication-results');
  if (!authResults) return 'unknown';
  if (AUTH_PASS_REGEX.test(authResults.value)) return 'pass';
  if (AUTH_FAIL_REGEX.test(authResults.value)) return 'fail';
  return 'unknown';
}
...
function parsedAddressList(addresses: Array<{ address?: string | null }> | undefined) {
  return (addresses ?? []).reduce<string[]>((acc, entry) => {
...
export async function receiveEmail(event: IncomingEmailMessage, env: Env): Promise<void> {
  ...
  if (allowedAddresses.length > 0) {
    ...
    if (!mailboxId) {
      logInfo('Ignoring email: no recipient matches EMAIL_ADDRESSES.');
      return;
    }
  } else {
  ...
  if (!(await env.BUCKET.head(mailboxKey(mailboxId)))) {
    logInfo('Ignoring email: mailbox does not exist');
    return;
  }
  ...
  const authStatus = senderAuthStatus(parsed.headers);
  if (authStatus !== 'fail' && !inReplyTo && references.length === 0) {
